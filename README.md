...
- (Optional) pynvim and lxml, for YouTube results
...
```
See the Configuration section for more information about `opts`.
...
Either a style string to use when drawing certain fields or a table controlling
how to draw properties in `format`.
Possible string values are `"unicode"`, `"ligature"`, and `"emoji"`.
The default value is `"unicode"`

See the "Rendering" section for more information.
...
The plugin should be fairly configurable when it comes to displaying data
from a player. When using a Lua config, the `style` option may also be a table.

The below example demonstrates the available options:

```lua
style = {
    -- Use default according to pictographic style.
    -- Currently, the only pictographic field is "pause".
    ["pause"] = "unicode",
    -- Use defaults regardless of pictographic style.
    -- "unicode" would have the same effect, since there is no "unicode"
    -- renderer for "duration", but there is a default one.
    ["duration"] = "any",
    -- User-defined renderer with automatic highlight binding (see next section).
    -- Thresholds are absolute, _strictly numeric_, and can contain one or two values.
    ["volume"] = {
        formatter = function(value) return tostring(value) .. "%" end,
        threshold = { 80 },
    },
    -- Lookup table based on property value.
    -- Values (in the key-value sense) must be 2-tuples of the display string and highlight name.
    -- Distinguished by the plugin from the prior item by the `formatter` field.
    ["loop"] = {
        ["inf"] = { "oo", "MpvLoopInfinite" }
    },
    -- Manual user-defined renderer.
    -- No binding of highlights is done, but the user is free to dynamically adjust
    -- both color and display at their leisure.
    ["playback-time"] = function(value)
        return { tostring(value), "MpvPlaybackTime" }
    end,
}
```

In the event that a renderer cannot be found in this table, the default is:

```lua
function default_renderer(value)
    -- See next section for more information about highlights
    return { tostring(value), "MpvPropertyName" }
end
```
...
When the style implies automatic highlight binding, the name of the highlight
is with "Mpv", followed by the property name. Properties in mpv
...
All properties which occur in the configuration's `format` field are given
highlights that link to bound to the plugin default highlight `MpvDefault`,
unless they have already been defaulted in the plugin.
The following defaults additional defaults exist:
...
When using a custom formatter with thresholds, the original highlight name for
the property will not be used. Instead, partitioned highlight names are used:
"Low", "Middle", and "High" for two-valued thresholds;
and "Low" and "High" for one-valued thresholds.
...
config = {
    ...
    style = {
        -- ...
        ["playback-time"] = { formatter = tostring, threshold = {10} }
    }
}
vim.cmd[[hi! link MpvPlaybackTimeLow ErrorMsg]]
```
