...
class MpvProtocol(asyncio.Protocol):
    ...
    def data_received(self, data):
        ...
        for datum in data.split(b"\n"):
            ...
            if datum.get("error") not in ("success", None):
                ...
                if (
                    property_name := self._reverse_properties.get(request_id)
                ) is not None:
                ...
                self._try_handle_event(
                    "got-playlist",
                    {"playlist": datum.get("data"), "new": self.playlist_new},
                )
                self._playlist_request = -1
                self.playlist_new = None
    ...
    async def wait_property(self, property_name, ignore_error=False):
        ...
        self.get_property(
            property_name, request_id=self._last_property, ignore_error=ignore_error
        )
        self._last_property += 1
        return await future
    ...
    def set_property(self, property_name, value, update=True, ignore_error=False):
        ...
        if not update:
            self.send_command(
                "set_property", property_name, value, ignore_error=ignore_error
            )
            return
