...
class Neovimpv:  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, nvim):
        ...
        self.mpv_socket_dir = os.path.join(
            os.path.dirname(tempname), self.__class__.__name__.lower()
        )
        os.makedirs(self.mpv_socket_dir)

        self._mpv_instances = {}

    def create_mpv_instance(
        self, lines, start, end, args, ignore_mode=False
    ):  # pylint: disable=too-many-arguments
        """Create an MpvManager and register it in `self._mpv_instances`"""
        if start == end and self.get_mpv_by_line(self.nvim.current.buffer, start):
    ...
    def mpv_youtube_search(self, args, _, bang=False):
        ...
        if WARN_LXML:
            self.show_error(
                "Python module lxml not detected. Cannot open YouTube results."
            )
            return
    ...
    def show_error(self, error, level=4):
        ...
        log.error(error)
        self.nvim.async_call(self.nvim.api.notify, error, level, {})
    ...
    def query_mpvs(self, arg):
        ...
        return self.get_mpvs_in_buffer(buffnum or self.nvim.current.buffer.number)
    ...
    def get_mpv_by_line(self, buffer, line):
        ...
        instance exists.
        """
        try_get_mpv = self.nvim.lua.neovimpv.get_player_by_line(buffer.number, line)
        if not try_get_mpv:
            return None
    ...
    def remove_mpv_instance(self, instance):
        ...
        try:
            ...
            self.nvim.lua.neovimpv.remove_player(instance.buffer, instance.id)
